...
function throttle(f, ms) {
  ...
  return function wrapper() {
    ...
    setTimeout(function() {
      canRun = true;
      // Проверка именно на currArgs, потому что currThis м.б.
      // всё равно равен null
      // currArgs не null, если декорированная функция запущена менее
      // чем через ms мс после последнего запуска
      if (currArgs) {
        // Вызываем не функцию, а обёртку, чтобы создать
        // задержку перед следующим запуском:
