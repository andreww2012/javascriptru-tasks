...
// Если мы хотим получить и последующие значения массива, но не уверены в их числе, надо
// использовать оператор "..." («spread», троеточие), который присвоит переменной массив с
...
// Если хочется присвоить свойство объекта в переменную с другим именем, например, чтобы свойство
// options.width пошло в переменную w, то можно указать соответствие через двоеточие.
...
// Деструктуризацию можно проводить без let, но тогда нужно оборачивать всё выражение (при
// деструктуризации объекта) в круглые (чтобы js не распознал их как блок {}):
let a, b;
({a, b} = {a:5, b:6});

// Поддерживается вложенная Деструктуризация, при деструктуризации нужно лишь сохранить
