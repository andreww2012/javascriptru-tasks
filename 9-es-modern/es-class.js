...
// Полная статья: http://learn.javascript.ru/es-class

// 1. Новый синтаксис для классов (по факту синтаксический сахар)
// Функция constructor запускается при создании new User, остальные методы
// записываются в User.prototype.
// При объявлении через class есть ряд отличий от классического объявления:
// - класс нельзя вызывать без new, будет ошибка;
// - объявление класса видно только в текущем блоке и только в коде, который
// находится ниже объявления.
...
// - все методы класса не перечислимы (в цикле for..in по объекту их не будет).
class User {
...
// User недоступна извне
let SiteGuest = class User { sayHi() { alert('Привет!'); } };

// 3. Геттеры, сеттеры, вычисляемые свойства
// В классах, как и в обычных объектах, можно объявлять геттеры и сеттеры через
// get/set, а также использовать [...] для свойств с вычисляемыми именами:
class User3 {
...
// - методы родителя можно переопределить в наследнике. Для обращения к
// родительскому методу используют super.walk();
// - конструктор родителя наследуется автоматически: если в потомке не указан
// свой constructor, то используется родительский;
// - если у потомка свой constructor, то чтобы в нём вызвать конструктор
// родителя – используется синтаксис super(), НО:
