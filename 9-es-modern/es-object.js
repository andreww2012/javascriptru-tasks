...
// Object.is(value1, value2) возвращает true, если значения value1 и value2 равны, иначе false.
// Отличия от ===:
...
// 5. Методы объекта
// Добавлены «методы объекта», которые, по сути, являются свойствами-функциями, привязанными
// к объекту. Их особенности:
// - более короткий синтаксис объявления;
// - наличие в методах специального внутреннего свойства [[HomeObject]], ссылающегося на объект,
...
// Пример: если rabbit наследует от animal, то вызов super.walk() из метода объекта rabbit
// обращается к animal.walk().
// При обращении через super используется [[HomeObject]] текущего метода, и от него берётся
// __proto__.
