...
// Записываю для себя. Полная статья: http://learn.javascript.ru/es-string
...
// Можно использовать свою функцию шаблонизации для строк. Название этой функции ставится перед
// первой обратной кавычкой.
// Эта функция будет автоматически вызвана в строке (*) и получит в качестве аргументов строку,
// разбитую по вхождениям параметров ${…} и сами эти параметры.
// Участки строки идут в первый аргумент-массив strings, аргументы - в дальнейший список аргументов
// values.
// В свойстве strings.raw находятся строки в точности как в оригинале (\n – это перевод строки, а в
// strings.raw – это \n)
...
let str = f`Sum of ${bananas} + ${pinapples} =\n ${bananas + pinapples}!`; // (*)

// 3. Улучшенная поддержка юникода
// Суррогатная пара - явление, когда одному символу языка соответствует два юникодных символа
// (4 байта вместо 2 байт)
// Добавлены методы String.fromCodePoint и str.codePointAt – аналоги String.fromCharCode и
// str.charCodeAt,
...
alert( String.fromCharCode(119987) ); // 풳, неверно (берёт первые два байта от числа)
alert( String.fromCodePoint(119987) ); // 𝒳, правильно

// 4. Синтаксис \u{длинный код}
// Чтобы вводить более длинные коды символов, добавили запись \u{NNNNNNNN}, где NNNNNNNN –
// максимально восьмизначный код.
alert( "\u{20331}" ); // 𠌱, китайский иероглиф с этим кодом

// 5. Unicode-нормализация
// Некоторые символы получаются с добавлением одного или неск. диакритических знаков. Пример:
// S с точками сверху и снизу.
// Проблема в том, что такой символ можно представить по-разному, отсюда - неравенство строк:
alert("S\u0307\u0323"); // Ṩ
alert("S\u0323\u0307"); // Ṩ (изменён порядок добавления точек)
alert( "S\u0307\u0323" == "S\u0323\u0307" ); // false
// С целью разрешить эту ситуацию, существует юникодная нормализация, при которой строки приводятся
// к единому, «нормальному», виду. В современном JavaScript это делает метод str.normalize().
