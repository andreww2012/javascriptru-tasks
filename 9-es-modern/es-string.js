...
// Участки строки идут в первый аргумент-массив strings, аргументы - в дальнейший список аргументов values.
// В свойстве strings.raw находятся строки в точности как в оригинале (\n – это перевод строки, а в strings.raw – это \n)
...
// Суррогатная пара - явление, когда одному символу языка соответствует два юникодных символа (4 байта вместо 2 байт)
// Добавлены методы String.fromCodePoint и str.codePointAt – аналоги String.fromCharCode и str.charCodeAt,
...
// Чтобы вводить более длинные коды символов, добавили запись \u{NNNNNNNN}, где NNNNNNNN – максимально восьмизначный код.
alert( "\u{20331}" ); // 𠌱, китайский иероглиф с этим кодом

// 5. Unicode-нормализация
// Некоторые символы получаются с добавлением одного или неск. диакритических знаков. Пример: S с точками сверху и снизу.
...
alert( "S\u0307\u0323" == "S\u0323\u0307" ); // false
// С целью разрешить эту ситуацию, существует юникодная нормализация, при которой строки приводятся к единому,
// «нормальному», виду. В современном JavaScript это делает метод str.normalize().
