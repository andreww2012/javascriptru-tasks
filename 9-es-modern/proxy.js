...
// 1. Понятие Proxy
// Прокси (proxy) – особый объект, смысл которого – перехватывать обращения к другому объекту и,
// при необходимости, модифицировать их. Синтаксис:
...
// Почти любая операция может быть перехвачена и обработана прокси до или даже вместо доступа к
// объекту target, напр.: чтение, запись, получение списка св-в, вызов ф-ии (если target ф-я) и т.п.
...
// * receiver – объект, к которому было применено присваивание. Обычно либо сам прокси, либо
// прототипно наследующий от него. Этот аргумент используется редко.
...
// Есть много применений Proxy, например, методы get/set позволяют реализовать доступ к
// произвольным свойствам, которых в объекте нет. Пример см. в статье про прокси на сайте.

// 2.2. has
// has срабатывает в операторе in и некоторых других случаях, когда проверяется наличие свойства.
// Синтаксис has аналогичен get. При отс. ловушки операция произв. напрямую над исходным объектом.
...
// Если же ловушка enumerate есть, то она будет вызвана с единственным аргументом target и сможет
// вернуть итератор для свойств.
...
// ловушек может содержать и другие полезные свойства и методы.
// Каждый перехватчик получает в аргументах target и дополнительные параметры в зависимости от типа.
// Если перехватчик в handler не указан, то операция совершается, как если бы была вызвана прямо
// на target.

// P.S. (из комментариев): с помощью прокси можно, например, организовать приватные свойства.
