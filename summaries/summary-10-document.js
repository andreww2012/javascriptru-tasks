...
// * Получает элемент, который находится на координатах (x,y) относительно окна
// (для координат вне окна возвращает null):
elem = document.elementFromPoint(x, y);

// 18. Координаты в документе: http://learn.javascript.ru/coordinates-document
// * В системе координат относительно страницы указываются координаты
// поотношению не к окну браузера, а к документу в целом (аналогия с CSS:
// коорд. отн. окна – это position:fixed, а отн. документа – position:absolute).
// * Готовой функции для получения координат элемента относительно страницы нет.
// Свзязь между системами координат, по которой можно написать свою функцию
// (названия условны): pageY = clientY + текущая вертикальная прокрутка.
// Реализации функций в статье.
// * 3-я система координат - координаты относительно всего экрана (НЕ окна
// браузера). Получение размеров:
screen.width; screen.height;
// * Доступная ширина/высота:
screen.availWidth; screen.availHeight;
// * Координаты левого верхнего угла браузера:
